...
:::note[Base user story 1 | Wrapped Generation]
...
As a user, I would like fun games embedded into my Spotify Wrapped that quiz me
on my own taste (finish the lyrics, guess your most listened to musician, etc.)
...
Enabling users to create an account, log in, and sign in from any device is one of the pillars that support all the functionality in our application. Using Firebase's authentication services, we are capable of integrating these functionality. Firebase provides all the backend functionality to ensure that credentials are protected and that the access to the platform can be done from any device and place. With Firebase handling authentication, our development team can focus on building features, knowing that user authentication remains secure and reliable across all devices.
